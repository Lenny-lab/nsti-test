...
function renderQuestion() {
  ...
  const dimension = dataset.framework.dimensions.find((item) => item.id === question.dimension);
...
function computeResult() {
  ...
  const axes = dataset.framework.dimensions.map((dimension) => ({
    ...dimension,
  ...
  const top = ranking[0];
  return {
    type: top.name_zh || top.name,
    title: `你最像 ${top.name_zh || top.name}`,
    oneLiner: `综合所有题目后，你当前的整体节奏最接近 ${top.name_zh || top.name}。这次结果会直接落到一个具体人物身上，而不是只给你一个抽象标签。`,
    axes,
...
function renderResult() {
  ...
  els.resultBadges.innerHTML = [
    `最像角色 · ${result.top.name_zh || result.top.name}`,
    `匹配度 · ${result.top.similarity}%`,
    `角色标签 · ${(result.top.traits_zh || result.top.traits || []).slice(0, 3).join(" / ") || "混合型"}`,
  ]
  ...
  els.resultPortrait.innerHTML = `
    <p>你的结果首先对应的是一个人物：<strong>${result.top.name_zh || result.top.name}</strong>。这意味着你的答题节奏、相处方式和行动倾向，在整体上最接近这个角色。</p>
    <p>如果再往下看，你的气质组合更偏“${result.archetype}”。所以这不是只看一题定结果，而是先做角色匹配，再用维度解释你为什么像这个人物。</p>
  `;

  els.resultCharacter.innerHTML = `
    <p><strong>${result.top.name_zh || result.top.name}</strong>${result.top.species_zh ? ` · ${result.top.species_zh}` : ""}</p>
    <p>${result.top.summary_zh || result.top.summary}</p>
    <div class="chipWrap">${(result.top.traits_zh || result.top.traits || []).map((trait) => `<span class="chip">${trait}</span>`).join("")}</div>
  ...
  els.resultTopMatches.innerHTML = result.ranking
    .map((item, index) => `<p>${index + 1}. ${item.name_zh || item.name} · ${item.similarity}%</p>`)
    .join("");
...
function copyResult() {
  ...
  const text = [
    ...
    `最像角色：${state.result.top.name_zh || state.result.top.name}`,
